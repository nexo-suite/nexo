...
export function registerWebhooks(webhooks: Webhooks, env: Env): void {
	...
	async function onContainerReady(
		...
		if (!m) {
			// Tag is often empty for manifest-creation events; only worth a debug
			// line when something looked PR-shaped but didn't parse, to keep noise
			// out of the routine main-build/manifest event stream.
			if (tag) logger.debug('package skipped: tag is not pr-N', { tag, app });
			return;
		}
