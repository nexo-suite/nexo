...
<main class="login-root">
	{#if data.user}
		<div class="login-card">
			...
			<h1 class="heading">{m.login_already_heading()}</h1>
			<p class="sub">{m.login_already_sub({ email: data.user.email })}</p>

			<div class="providers">
				<a href={appsUrl} class="provider-btn provider-btn--primary">
					<svg
						class="provider-icon"
						viewBox="0 0 24 24"
						fill="none"
...
<style>
	...
	.provider-btn--primary:hover {
		...
		border-color: color-mix(in oklab, var(--color-accent) 85%, #000);
	}
</style>
